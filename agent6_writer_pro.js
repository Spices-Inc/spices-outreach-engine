...
Since you're running {{company}} out of {{city}} near our Northumberland, PA facility, you will typically see your order at your kitchen door {{transit_days_text}}.
...
You can't control a subscriber's New Year's motivation, but you can protect the line in your kitchen. When a primary vendor leaves you short on a signature blend or a key chile powder, it forces an immediate recipe change—or worse, a meal substitution.

An unexpected flavor change in a previously purchased meal can be the final push-out-the-door for a wavering customer.

We often start as an emergency backup spice supplier, but meal prep companies stay with us because of our:

- 97%+ In-Stock Rate: Deep inventory on staples and blends (Tikka Masala, Shawarma, Taco) so you can avoid substitutions.

- 3:30 PM Same Day Shipping Guarantee: Orders placed by mid-afternoon typically arrive at your kitchen in {{city}} {{transit_days_text}}.
...
I'll leave my contact info below. Based on your location, we can typically get product to your door {{transit_days_text}}. If a primary vendor ever leaves you short, we're ready to help you protect your production schedule with that turnaround and the nutritional panels and allergen statements to match.
...
function getRotationLine(rotationDay) {
  if (rotationDay && rotationDay !== 'weekly' && rotationDay !== null) {
    const day = rotationDay.charAt(0).toUpperCase() + rotationDay.slice(1);
...
function populateTemplate(template, lead) {
  const firstName = lead.contact_name.split(' ')[0];
  const company = lead.company_name.split(':')[0].split('-')[0].trim();
  const rotationLine = getRotationLine(lead.rotation_day);
  
  return template
    .replace(/{{first_name}}/g, firstName)
    .replace(/{{company}}/g, company)
    .replace(/{{city}}/g, lead.city || 'your area')
    .replace(/{{transit_days_text}}/g, lead.transit_days_text || 'within two days')
    .replace(/{{rotation_line}}/g, rotationLine);
}

async function run() {
  console.log("\n✍️  Agent 6: Generating email sequences...\n");
  
  const leads = JSON.parse(fs.readFileSync('qualified_leads.json', 'utf8'));
  
  if (leads.length === 0) {
    console.log("  ⚠️  No qualified leads to process.\n");
    return;
  }
  ...
  for (const lead of leads) {
    ...
    console.log(`     Rotation: ${lead.rotation_day || 'weekly'}`);
    console.log(`     Score: ${lead.qualification_score}/100 (${lead.tier})`);
    ...
    lead.emails = emails;
    console.log(`     ✅ Generated 5 emails\n`);
  }
  
  fs.writeFileSync('final_leads_for_pipedrive.json', JSON.stringify(leads, null, 2));
  console.log(`✅ Complete! ${leads.length} leads ready for Pipedrive.\n`);
}
